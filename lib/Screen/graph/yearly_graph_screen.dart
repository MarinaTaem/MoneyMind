...
class YearlyGraphScreen extends StatefulWidget {
  ...
  YearlyGraphScreen({super.key, required this.userId, this.transaction});

  @override
  State<YearlyGraphScreen> createState() => _YearlyGraphScreenState();
}

class _YearlyGraphScreenState extends State<YearlyGraphScreen> {
  late List<Transaction> transactions;
  late List<double> income = List.filled(4, 0);
  late List<double> expense = List.filled(4, 0);
  late Budget budget;
  ...
  Future<void> _loadTransactions() async {
    ...
    // Use calculateYearlyTotals here
    final yearlyTotals = calculateYearlyTotals(transactions);

    // Get sorted years for reference (optional, for labeling)
    // late List<int> sortedYears = [];

    setState(() {
      income = yearlyTotals['income'] ?? [];
      expense = yearlyTotals['expense'] ?? [];
      sortedYears = transactions.map((t) => t.dateRecord.year).toSet().toList()
        ..sort();
    });
  }
  ...
  Widget _bottomTitles(double value, TitleMeta meta) {
    print('bottomTitles value: $value, sortedYears: $sortedYears');
    String text = '';
    if (value.toInt() >= 0 && value.toInt() < sortedYears.length) {
      text = sortedYears[value.toInt()].toString();
    }
